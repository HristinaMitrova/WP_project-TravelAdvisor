...
public class Agency {
    ...
    @JoinTable(name="agency_offers",
            joinColumns = @JoinColumn(name="agencyName"),
            inverseJoinColumns = @JoinColumn(name = "offer"))
    private List<Arrangement> offers;

    public Agency(long id,String agencyName,String location,String contactNo,String contactMail,List<Review> reviewList,List<User> userList,List<Arrangement> offers){
        this.id=id;
